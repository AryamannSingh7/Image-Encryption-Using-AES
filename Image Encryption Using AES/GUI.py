...
def key():
    ...
    elif(enc_v.get()=="Pixel Shuffle"):
        enc_key=simpledialog.askinteger("password","Enter the block size(ps:50-100)")
    else:
...
def Deckey():
    ...
    elif(dec_v.get()=="Pixel Shuffle"):
        dec_key=simpledialog.askinteger("password","Enter the block size(ps:50-100)")
    else:
        messagebox.showwarning("warning","No method selected")
...
def encrypt():
    ...
    if(enc_v.get()=="AES-256"):
        ...
        parent_=os.path.join(parent_dir,"encrypt/output.jpg")
        PIXEL_SHUFFLE.shuffle_encrypt(parent_,enc_key,enc_file)
        tk.messagebox.showinfo("Success","Encryption Successful")
    else:
        print(enc_v.get())
...
def decrypt():
    ...
    if(dec_v.get()=="AES-256"):
        ...
        parent_=os.path.join(parent_dir,"decrypt/output.jpg")
        PIXEL_SHUFFLE.shuffle_encrypt(parent_,dec_key,dec_file)
        tk.messagebox.showinfo("success","Decryption Successful")
    else:
        print(dec_v.get())
...
def frame():
    ...
    top.title("Image Encryption By Aryamann Singh")
    labelframe2=tk.LabelFrame(top,text="Image Encryption: ",height=175,width=470,bg="black").place(x=10,y=10)
    labelframe3=tk.LabelFrame(top,text="Image Decryption: ",height=185,width=470,bg="black").place(x=10,y=205)
    
    global i
    global enc_v
    global enc_key
    global enc_file
    listus=["AES-256","Pixel Shuffle"]
    enc_v=tk.StringVar()
    enc_v.set("None")
    i=tk.StringVar()
    ...
    inputEncBtn = tk.Button(labelframe2, text="Browse ...",width=10, height=1,bg="red",command=openfileEnc).place(x=395,y=46)
    ...
    keyBtn=tk.Button(labelframe2,text="Input Key", height=1,activebackground="black",command=key).place(x=135,y=130,width=100)
    ...
    inputDecBtn = tk.Button(labelframe2, text="Browse ...",width=10, height=1,bg="red",command=openfileDec).place(x=395,y=241)
    ...
    keyBtn=tk.Button(labelframe2,text="Input Key", height=1,activebackground="black",command=Deckey).place(x=135,y=325,width=100)
